...
optimizer = torch.optim.Adadelta(mlp_classifier.parameters(), lr= learning_rate)
...
for epoch in range(epochs):
    ...
    for batch_sentences, batch_labels in tqdm(train_dataloader):
        ...
        classifier_outputs = mlp_classifier(batch_embeddings)
    ...
    with torch.no_grad():
        ...
        val_loss = []
        step_count = 0
        for batch_sentences, batch_labels in tqdm(test_dataloader):
            batch_labels = (batch_labels).float().to(device)
            batch_embeddings = []
            for sentence in batch_sentences:
...
metrics_df.to_csv(f"results-{datetime.datetime.now().strftime('%Y-%m-%d')}-e-{epochs}.csv", index=False)
writer.close()
